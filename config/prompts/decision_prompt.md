...
**当BUG文档不及格时**:
- **先检查问题是否已解决**: 如果消息中包含"已修复"、"已解决"、"验证通过"、"✅ 已通过"等关键词 → 问题已闭环 → `action: ignore`
- **未解决且不及格**: 通知验收群 `@qa 请重新生成完整的BUG报告，缺少必要信息`
...
```
# 第一步：检查问题是否已经解决
if 消息中明确说明问题已解决（"已修复"、"已解决"、"验证通过"、"✅"）:
    if 目标群也确认 → wait/ignore（已闭环）
    else → notify（通知目标群"问题已解决"）
...
- `bug_doc_complete`: 检查BUG报告是否完整，**必须先判断问题是否已解决**
  - **问题已解决**（消息含"已修复"/"验证通过"/"✅"）→ **必须设为true**，不要通知QA重新生成
  - `true`: 文档完整 或 问题已解决，可以通知开发/运维处理
  - `false`: 文档不完整 **且** 问题尚未解决，需要通知QA重新生成
