...
1. **操作步骤**: 具体做了什么操作（请求方法、API路径、参数）
2. **请求参数**: 完整的请求body/query/path参数
3. **实际结果**: 实际返回的状态码、响应体、错误信息
4. **期望结果**: 应该返回什么
...
- 实际结果: 404 Not Found, {"error": "Cannot GET /api/v1/comments/comment-123/history"}
- 期望结果: 200 OK, 返回评论历史记录数组
```

**当BUG文档不及格时**:
- **行动**: 通知验收群 `@qa 请重新生成完整的BUG报告，当前报告缺少必要信息`
- **要求**: 必须包含操作步骤、请求参数、实际结果、期望结果
