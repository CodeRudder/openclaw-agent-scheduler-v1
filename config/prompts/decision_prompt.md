...
**检查清单**:
1. QA是否测试了**每个页面**？（列出页面清单）
2. QA是否测试了**每个API接口**？（列出API清单）
3. 验收报告是否包含完整的测试用例清单？

**决策规则**:
```
if 所有群都无待执行任务:
    if 验收群最新消息是"验收通过"但未提及测试覆盖范围:
        return notify → 验收群 @product @qa
        message: "请确认验收完整性：1)是否覆盖每个页面？2)是否覆盖每个API？3)请提供完整测试用例清单"
    else:
        return ignore
```
